...
def fig6_structural_environment(
    ...
    Pe = 1025 * 9.81 * d / 1e5
    ax.plot(d, Pe, color=LAYER_CLR[1], lw=2.0, label="Hydrostatic P = rho·g·h")
    ...
    ax.set_ylabel("Cumulative Corrosion (mm)")
    ...
    Q_range = [
        phys.Cd * np.pi * (d_pin / 2) ** 2 * np.sqrt(2 * phys.dP_orifice / phys.rho_oil) * 1000
        for d_pin in d_range
    ]
    ax.plot(d_range * 1000, Q_range, color=C_SENSOR, lw=2.0,
            label="Q = Cd·A·sqrt(2dP/rho) [ISO 5167, Ref 6]")
    ...
    ax.set_ylabel("Max leak flow (L/s)")
    ...
    ax.semilogx(freq, snr_q, color=C_SENSOR, lw=1.5, ls="--", label="Quartz only (ref element)")
    ax.semilogx(freq, snr_h, color=LAYER_CLR[4], lw=1.5, ls=":", label="Hydrophone only")
    ...
    _save_split_panels(fig, [
        ("Fig6_Pressure_vs_Depth.svg", [axes[0, 0]]),
        ("Fig6_Corrosion_Resistance.svg", [axes[0, 1]]),
        ("Fig6_Pinhole_Sensitivity.svg", [axes[1, 0]]),
        ("Fig6_Acoustic_SNR_vs_Frequency.svg", [axes[1, 1]]),
    ])
    _save(fig, "Fig6_Structural_Environment.svg")
